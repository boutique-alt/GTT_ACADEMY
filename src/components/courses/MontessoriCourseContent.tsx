...
export default function MontessoriCourseContent() {
  const { pageIntro, image, title, levels, shared } = montessoriCourse;
  ...
  const accordionItems = [
    ...
    ...(level.extraSections?.map((section) => ({
      title: section.title,
      content: (
    ...
    },
    {
      ...
      content: (
        <div className="space-y-5">
          {level.courseContentIntro ? <p>{level.courseContentIntro}</p> : null}
    ...
    },
    {
      ...
      content: (
        <div className="space-y-4">
          {level.certification.map((paragraph) => (
            <p key={paragraph.slice(0, 48)}>{paragraph}</p>
          ))}
        </div>
      ),
    ...
    },
    {
      ...
      content: (
        <div className="space-y-4">
          <CourseBulletList items={shared.apostilleSteps} />
          {shared.apostilleNote ? <p>{shared.apostilleNote}</p> : null}
        </div>
      ),
    },
    {
      ...
      >
        <div className="min-w-0 max-w-full rounded-[1.5rem] border border-slate-100 bg-white p-5 shadow-sm sm:p-8 lg:p-8">
          ...
          <div className="relative mt-7 aspect-[16/10] w-full overflow-hidden rounded-2xl bg-slate-100">
            <Image src={image} alt={title} fill className="object-cover" sizes="(max-width: 1024px) 100vw, 640px" priority />
          </div>
