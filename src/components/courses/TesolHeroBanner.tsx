...
export default function TesolHeroBanner({
  image,
  ariaLabel,
  title,
  titleClassName = "text-center font-display text-2xl font-semibold tracking-[-0.02em] text-white sm:text-3xl lg:text-[3.25rem] lg:leading-[1.1] xl:text-[4rem]",
}: Props) {
  return (
    <>
      <section className="relative isolate w-full overflow-hidden bg-[#05245b]">
        <div className="relative aspect-[4/3] w-full sm:aspect-video">
          ...
          />
          <div className="relative z-10 flex h-full w-full min-w-0 items-center">
            <div className="flex w-[min(78%,18rem)] max-w-full items-center px-4 py-8 sm:w-[min(52%,32rem)] sm:px-8 lg:w-[min(58%,48rem)] lg:px-12 xl:w-[min(60%,52rem)] xl:px-16">
              <QuoteBlock />
            </div>
          </div>
        </div>
