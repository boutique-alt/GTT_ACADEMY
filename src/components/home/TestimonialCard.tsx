...
export default function TestimonialCard({ name, text, image, copy }: Props) {
  return (
    <article className="group/card flex h-[470px] w-[260px] shrink-0 flex-col overflow-hidden rounded-3xl border border-white/15 bg-white shadow-2xl shadow-blue-950/25 transition-[width,box-shadow] duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] hover:w-[540px] hover:flex-row hover:shadow-[0_28px_60px_-16px_rgba(5,36,91,0.45)] sm:h-[490px] sm:w-[290px] sm:hover:w-[580px]">
      <div className="relative h-[42%] w-full shrink-0 overflow-hidden bg-[#f3f7fc] transition-[height,width] duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover/card:h-full group-hover/card:w-[250px] sm:group-hover/card:w-[270px]">
        <Image
          src={image}
          alt={name}
          fill
          sizes="(max-width: 640px) 260px, 290px"
          className="object-contain transition duration-700 ease-out group-hover/card:scale-[1.04]"
        />
        ...
        <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-[#05245b]/20 via-transparent to-transparent opacity-0 transition duration-500 group-hover/card:opacity-100" />
      </div>

      <div className="flex min-h-0 flex-1 flex-col justify-between gap-3 p-5 pb-6 transition-[padding] duration-500 sm:p-6 sm:pb-7 group-hover/card:justify-center group-hover/card:p-6 sm:group-hover/card:p-8">
